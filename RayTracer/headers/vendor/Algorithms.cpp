...
int partition(IntersectData* arr, int start, int end) {
	IntersectData pivot = arr[end]; // Pivot is at end(right side of partition)
	int i = start - 1;
	...
	for(int j = start; j < end; j++) {
		if(arr[j].t < pivot.t) {
			i++;
			// Swap the values
			IntersectData temp = arr[j];
			arr[j] = arr[i];
			arr[i] = temp;
		}
	}
	// Swap the values
	IntersectData temp = arr[end];
	arr[end] = arr[i + 1];
	arr[i + 1] = temp;
